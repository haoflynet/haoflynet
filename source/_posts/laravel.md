...
User::where(...)->get()->pluck('name')	# 只取某个字段的值，而不是每条记录取那一个字段，这是平铺的,这里的pluck针对的是一个Collection
...
Model::where('column', 'LIKE', '%value%')->get()	# LIKE语句
...
$author->posts()->save($post);	# 添加hasone或者hasmany
...
使用命令的方式执行脚本，这时候如果要打印一些信息，可以直接用预定义的方法，还能显示特定的颜色:
...
Laravel日志工具在强大的`Monolog`函数库上提供一层简单的功能，Laravel默认为应用建立每天的日志文件在`storage/logs`目录，同样有七个等级的日志级别：debug、info、notice、warning、error、critical和alert
