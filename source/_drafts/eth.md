...
- 区块链是一种特殊的分布式数据库，只能做插入和查找操作
- 任何人都可以架设服务器加入区块链网络，成为一个节点，并没有中心节点，每个节点都是平等的，都保存着整个数据库
...
- Gas Price: 是指Gas的单价，指每个Gas值多少的Ether，单价越高，矿工更愿意优先打包你的交易，你的交易就能更快完成
- Gas Limit：是指本次交易花费的Gas的上限，不一定会花完，剩下的会返回到账户中，通过提高这个值能够解决`Out of Gas(OOG)`的问题，但是如果超过创世块设置的最大值也会报错`exceeds block gas limit`
...
| ----------------------- | --------- | ------------------------- |
| wei                     | 1 wei     | 1                         |
| ether，以太币的最小单位 | 1^18 wei  | 1,000,000,000,000,000,000 |
